...
/** Rigenera il tab ReportTemplate con i dati aggregati (ore, assenze, spese) del mese/anno scelto. */
export async function generateMonthlyReport(
  ...
  const [timeEntries, absenceEntries, expenseEntries] = await Promise.all([
    ...
    listExpenseEntries(accessToken, spreadsheetId),
  ]);
  ...
  if (monthExpenses.length === 0) {
    ...
    for (const e of monthExpenses) {
      rows.push([e.date, e.missionRef, e.amount, e.description, REIMBURSEMENT_STATUS_LABELS[e.reimbursementStatus], e.reimbursedAmount]);
    }
  }
